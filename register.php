...
<head>
    ...
    if (isset($_POST["submit"])) {
        ...
        if (sizeof($errors) == 0) {
            ...
            try {
                $query = "INSERT INTO `customer`(`sessKey`, `name`, `address1`, `address2`, `password`, `email`, `phone`, `BirthDate`) VALUES ('$key','$_POST[firstName] $_POST[lastName]','$_POST[address]','$address2','$password','$_POST[email]','$_POST[phone]','$_POST[birthDate]')";
                ...
                if(preg_match("/'email'$/", mysqli_error($connector->conn))){
                    $errors["email"] = "E-mail is already taken";
                }
                else if(preg_match("/'phone'$/", mysqli_error($connector->conn))){
...
<body>
    ...
    <form method="post" class="p-4 w-75 my-5 m-auto d-grid" style="background-color: #f1f1ec;">
        ...
        <div class="row">
            ...
            <div class="col-12 col-md-6">
                <label class="form-label font" for="address2">Another Address (Optional)</label>
                <input class="form-control mb-3" type="text" name="address2" id="address2" value="<?= htmlspecialchars($_POST["address2"] ?? " ") ?>">
            </div>
        </div>
        <div class="row">
            <div class="col-12 col-md-6">
