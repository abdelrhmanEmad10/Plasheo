...
$name=$_GET['name'];
$sql = "SELECT product_id, name, price, `img-path`, description, rate FROM product WHERE LOWER(name) LIKE '%$name%'";
$sort_options = [
  '' => 'Default sorting',
  'pop' => 'Sort by Popularity',
  'avg' => 'Sort by avarage rating',
  'lst' => 'Sort by latest',
  'lth' => 'Sort by price: low to hight',
  'htl' => 'Sort by price: hight to low'
];
...
<head>
  <title>PLASHOE || Search Shoe</title>
  <meta charset="UTF-8">
...
<body>
  <div class="container mb-4" id="page">
    ...
    <div class="row row-cols-1 row-cols-md-2">
      <div class="col ff">
        ...
        <div class="offcanvas offcanvas-start" data-bs-scroll="true" tabindex="-1" id="side" aria-labelledby="offcanvasWithBothOptionsLabel">
          ...
          <div class="offcanvas-body">
            ...
            <div class="wrapper">
              <form id="form1" action="http://localhost/project/searchPage.php?name=<?=$name?>&" method="get">
                <div class="slider">
                ...
                <div class="price-input">
                  <div class="field btn">
                    <span>$</span>
                    <input name="input_min" onchange="submitFormOnChange()" type="text" id="input_min" class="input-min" value="40">
                  </div>
                  <div class="field btn">
                    <span>$</span>
                    <input name="input_max" onchange="submitFormOnChange()" type="text" id="input_max" class="input-max" value="110">
                  </div>
                </div>
            ...
            <div class="categories mt-4">
              <ul>
                <li><a href="./Men.php">Men</a> <span style="color:rgb(151,154,155) ;">(<?php
                                                                                $sql2 = "SELECT product_id, name, price, `img-path`, description, rate FROM product WHERE category_id =1";
                                                                                $result1 = $conn->query($sql2);
                                                                                echo $result1->num_rows;
                                                                                ?>)</span>
                </li>
                <li><a href="./women.php">Women</a> <span style="color:rgb(151,154,155) ;">(<?php
      ...
      <div class="col ll justify-content-start ">
        <form id="form2" action="http://localhost/project/searchPage.php?name=<?=$name?>&">
    ...
    if ((isset($_GET['input_min']) || isset($_GET['input_max']))&&($minPrice != 40||$maxPrice != 110)) {
      ...
      if (isset($_GET['input_min'])&&$minPrice != 40) {
        echo "<a style='text-decoration:none;' href='./searchPage.php?name=$name' class='fil' onclick='updateMin()'>
    ...
    Min $" . number_format($minPrice, 2) . '</a>';
      ...
      if (isset($_GET['input_max'])&&$maxPrice != 110) {
        echo "<a style='text-decoration:none;' href='./searchPage.php?name=$name' class='fil' onclick='updateMax()'>
      <span class='clear'><svg xmlns='http://www.w3.org/2000/svg' width='20' height='20' fill='rgb(110,112,81)' class='bi bi-x-circle-fill' viewBox='0 0 16 16'>
      <path d='M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0M5.354 4.646a.5.5 0 1 0-.708.708L7.293 8l-2.647 2.646a.5.5 0 0 0 .708.708L8 8.707l2.646 2.647a.5.5 0 0 0 .708-.708L8.707 8l2.647-2.646a.5.5 0 0 0-.708-.708L8 7.293z'/>
    </svg></span>
    ...
    if ($result->num_rows > 0) {
      while ($row = $result->fetch_assoc()) {
        ...
        $rating = str_repeat('★', $row["rate"]) . str_repeat('☆', 5 - $row["rate"]);
      ...
      echo "<div class='container mt-3'>
          <div class='row zfound'><p>No products were found matching your selection.</p></div>
    </div>";
    }
  ...
  <script>
    function submitFormOnChange() {
      ...
      var input_max = document.getElementById("input_max").value;
      var input_min = document.getElementById("input_min").value;

      let form = document.getElementById("form2")
      // Check if there are existing parameters in the URL

      // Update the form action by appending the selected value to the existing URL
      form.action = form.action + "input_max=" + encodeURIComponent(input_max) + '&' + "input_min=" + encodeURIComponent(input_min);
    ...
    function submitFormOnChange2() {
      ...
      // Update the form action by appending the selected value to the existing URL
      form.action = form.action + "sort=" + encodeURIComponent(selectedValue);
      location.replace(form.action)
